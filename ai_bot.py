...
# ===== ОТМЕНА ЗАПРОСОВ =====
# user_id -> Event (установлен = отменено)
...
def md_to_html(text: str) -> str:
    ...
    # Экранируем HTML-символы ПЕРЕД тем как добавить теги
    ...
    # ```язык\nкод``` → <pre><code class="language-язык">
...
def send_long(chat_id, text, reply_markup=None):
    ...
    chunk_size = 4000
    if len(converted) <= chunk_size:
        try:
            bot.send_message(chat_id, converted, parse_mode="HTML", reply_markup=reply_markup)
        except Exception:
            # Если HTML не прошёл — отправляем чистый текст
            bot.send_message(chat_id, text, reply_markup=reply_markup)
    else:
        parts = [converted[i:i+chunk_size] for i in range(0, len(converted), chunk_size)]
        for i, part in enumerate(parts):
            try:
                bot.send_message(chat_id, part, parse_mode="HTML",
                                 reply_markup=reply_markup if i == len(parts)-1 else None)
            except Exception:
                bot.send_message(chat_id, text[i*chunk_size:(i+1)*chunk_size],
                                 reply_markup=reply_markup if i == len(parts)-1 else None)
...
@restricted
def youtube_menu(msg):
    bot.send_message(msg.chat.id, "🎬 Отправь ссылку на YouTube видео:")
    bot.register_next_step_handler(msg, youtube_get_url)

def youtube_get_url(msg):
    if msg.from_user.id != ALLOWED_USER_ID: return
    url = msg.text.strip()
    if not url.startswith("http"):
        bot.send_message(msg.chat.id, "❌ Это не ссылка.", reply_markup=main_keyboard())
        return
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton("🎥 Видео (MP4)", callback_data=f"yt_video:{url}"),
        types.InlineKeyboardButton("🎵 Аудио (MP3)", callback_data=f"yt_audio:{url}")
    )
    bot.send_message(msg.chat.id, f"Что скачать?\n{url}", reply_markup=kb)

@bot.callback_query_handler(func=lambda call: call.data.startswith("yt_video:") or call.data.startswith("yt_audio:"))
def youtube_download_cb(call):
    if call.from_user.id != ALLOWED_USER_ID:
        bot.answer_callback_query(call.id, "Нет доступа"); return
    action, url = call.data.split(":", 1)
    bot.answer_callback_query(call.id, "Начинаю скачивание...")
    status = bot.send_message(call.message.chat.id, "⏳ Скачиваю...")
    os.makedirs(TMP_DIR, exist_ok=True)
    try:
        if action == "yt_video":
            cmd = ["yt-dlp", "-f", "best[ext=mp4]/best", "--merge-output-format", "mp4",
        ...
        if r.returncode != 0:
            bot.edit_message_text(f"❌ Ошибка скачивания:\n{r.stderr[:500]}", call.message.chat.id, status.message_id)
            return

        # Найти скачанный файл
        ext = "mp4" if action == "yt_video" else "mp3"
        files = [f for f in os.listdir(TMP_DIR) if f.endswith(f".{ext}")]
        if not files:
            bot.edit_message_text("❌ Файл не найден после скачивания.", call.message.chat.id, status.message_id)
            return

        filepath = os.path.join(TMP_DIR, files[-1])
        size_mb = os.path.getsize(filepath) / (1024 * 1024)

        bot.edit_message_text(f"✍️ Отправляю ({size_mb:.1f} МБ)...", call.message.chat.id, status.message_id)

        with open(filepath, "rb") as f:
            if action == "yt_video":
                bot.send_video(call.message.chat.id, f, caption=f"🎥 {files[-1]}", reply_markup=main_keyboard())
            else:
                bot.send_audio(call.message.chat.id, f, caption=f"🎵 {files[-1]}", reply_markup=main_keyboard())

        bot.delete_message(call.message.chat.id, status.message_id)
        os.remove(filepath)

    except subprocess.TimeoutExpired:
        bot.edit_message_text("❌ Тайм-аут: видео слишком длинное или сервер медленный.", call.message.chat.id, status.message_id)
    except Exception as e:
        try:
            bot.edit_message_text(f"❌ Ошибка: {e}", call.message.chat.id, status.message_id)
        except Exception:
            pass
...
def help_cmd(msg):
    bot.send_message(msg.chat.id, (
        ...
        "📂 Мои чаты — список и переключение\n"
        "📊 Статистика — токены, сообщения\n"
        "📤 Экспорт — сохранить чат в файл\n"
        "📥 Импорт — восстановить чат из файла\n"
        "⚙️ Настройки — промпт, резюме, шеринг\n"
        "🔍 Поиск — поиск по истории\n"
        "🎬 YouTube — скачать видео или аудио\n"
        "❓ Помощь — это сообщение\n\n"
        "Во время ответа ИИ появится кнопка ❌ Отменить."
...
def _cancel_kb():
    ...
    kb.add(types.InlineKeyboardButton("❌ Отменить", callback_data="cancel_request"))
    return kb

def _ask_with_search(user_text, cid, chat_id=None, image_data=None, mime_type=None):
    uid = ALLOWED_USER_ID
    cancel_event = threading.Event()
    _cancel_events[uid] = cancel_event

    status_msg = None
    if chat_id:
        try:
            status_msg = bot.send_message(chat_id, "⏳ Думаю...", reply_markup=_cancel_kb())
        except Exception:
            pass
    ...
    if cancel_event.is_set():
        if status_msg:
            try: bot.edit_message_text("❌ Отменено.", chat_id, status_msg.message_id)
            except Exception: pass
        _cancel_events.pop(uid, None)
        return None

    if af.needs_search(user_text):
        if status_msg:
            try: bot.edit_message_text("🔍 Ищу в интернете...", chat_id, status_msg.message_id, reply_markup=_cancel_kb())
            except Exception: pass
    ...
    if status_msg:
        try: bot.edit_message_text("✍️ Печатаю...", chat_id, status_msg.message_id, reply_markup=_cancel_kb())
        except Exception: pass
...
def cancel_request(call):
    if call.from_user.id != ALLOWED_USER_ID: return
    uid = ALLOWED_USER_ID
    if uid in _cancel_events:
        _cancel_events[uid].set()
    try:
...
def handle_text_v2(msg):
    ...
    answer = _ask_with_search(user_text, cid, chat_id=msg.chat.id)
    if answer is None:
        return  # отменено
    if c[5] == 0:
...
def handle_photo_v2(msg):
    ...
    answer = _ask_with_search(caption, cid, chat_id=msg.chat.id, image_data=downloaded, mime_type="image/jpeg")
    if answer is None:
        return  # отменено
    send_long(msg.chat.id, answer, reply_markup=main_keyboard())
...
def handle_doc_v2(msg):
    ...
    answer = _ask_with_search(caption or "Обработай файл.", cid, chat_id=msg.chat.id, image_data=downloaded, mime_type=msg.document.mime_type or "application/octet-stream")
    if answer is None:
        os.remove(tmp_path)
        return  # отменено
    send_long(msg.chat.id, answer, reply_markup=main_keyboard())
